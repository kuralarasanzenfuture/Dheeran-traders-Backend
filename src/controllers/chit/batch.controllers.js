...
//     if (!start_date || !end_date || !batch_duration) {
//       return res.status(400).json({ message: "Required fields missing" });
//     }
...
//     // 🔢 generate next batch name
//     const [rows] = await db.query(`
//       SELECT MAX(CAST(SUBSTRING(batch_name, 7) AS UNSIGNED)) AS last_no
//       FROM batches
//       WHERE batch_name LIKE 'Batch_%'
//     `);
...
//         updated_at,
//         CASE
//           WHEN CURDATE() < DATE(start_date) THEN 'WAITING'
//           WHEN CURDATE() BETWEEN DATE(start_date) AND DATE(end_date) THEN 'ACTIVE'
//           ELSE 'CLOSED'
//         END AS status
//       FROM batches
//       WHERE id = ?
...
//     // 🔒 closed check
//     if (new Date(batch[0].end_date) < new Date()) {
//       return res
//         .status(403)
//         .json({ message: "Closed batch cannot be modified" });
//     }
...
//     // 🔒 closed batch not editable
//     if (new Date(batch[0].end_date) < today) {
//       return res.status(403).json({
//         message: "Closed batch cannot be modified",
//       });
//     }
...
//       return res
//         .status(403)
//         .json({ message: "Closed batch cannot be deleted" });
//     }
...
export const createBatch = async (req, res) => {
  ...
  try {
    ...
    const { batch_duration, start_date, end_date, remarks } = req.body;
    ...
    // 🔢 generate batch name (⚠️ race condition still possible)
    ...
    const [result] = await connection.query(
      `INSERT INTO batches (batch_name, batch_duration, start_date, end_date, created_by)
       VALUES (?, ?, ?, ?, ?)`,
      [batch_name, batch_duration, start_date, end_date, userId]
    );

    const [newBatchRows] = await connection.query(
      `SELECT * FROM batches WHERE id = ?`,
      [result.insertId]
    );

    const newData = newBatchRows[0];
...
export const updateBatch = async (req, res) => {
  ...
  try {
    ...
    if (start > end) {
      return res.status(400).json({ message: "Start > End invalid" });
    }

    if (start < today) {
      return res.status(400).json({ message: "Past start date not allowed" });
    }

    // 🔒 closed batch
    if (new Date(oldData.end_date) < today) {
      return res.status(403).json({ message: "Closed batch cannot be modified" });
    }

    // ❌ overlap check (same as yours)
    const [overlap] = await connection.query(
      `SELECT id FROM batches
       WHERE id != ?
       AND (
    ...
    if (overlap.length) {
      return res.status(409).json({ message: "Batch overlap" });
    }

    // ✅ update
    await connection.query(
      `UPDATE batches
       SET batch_duration=?, start_date=?, end_date=?, updated_by=?
       WHERE id=?`,
      [batch_duration, start_date, end_date, userId, id]
    );
...
export const deleteBatch = async (req, res) => {
  ...
  try {
    ...
    // 🔍 get full data (IMPORTANT for audit)
    const [rows] = await connection.query(
      `SELECT * FROM batches WHERE id = ?`,
      [id]
    );
    ...
    // 🔒 closed batch check
    if (new Date(oldData.end_date) < new Date()) {
      return res.status(403).json({
        message: "Closed batch cannot be deleted"
      });
    }
    ...
    await AuditLog({
      ...
      oldData,
      userId,
      remarks: remarks || "Batch hard deleted"
    });
